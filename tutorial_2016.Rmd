...
ggplot(dentus_A_rot, aes(x, y)) + geom_point() + 
  ...
  geom_hline(yintercept = 0, color = "blue", linetype = "dashed") + theme_bw()
```
...
ggplot(dentus_A_rot_PCs, aes(x, y)) + geom_point() + 
  ...
  geom_hline(yintercept = 0, color = "green", linetype = "dashed") + theme_bw()
```
...
Vocês leram sobre linhas de menor resistencia evolutiva no artigo do Schluter, e agora podemos entender porque essa linha corresponde ao primeiro componente principal. A quantidade de variação total na população é sempre preservada em todas essas analises que fizemos, afinal são só rotações. Para se convencer, vamos somar as variâncias originais e somar as variâncias saindo da eigen():
