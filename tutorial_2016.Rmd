...
Existem dois métodos populares e faceis de usar no R para reconstrução de caracteres ancestrais: a parcimônia quadrada e a máxima verossimilhança. Ambos dão o mesmo resultado quando a filogenia usada não inclui os tamanhos de ramo. Cada método tem sua lógica e suas vantágens e desvantágens:
...
Agora eu vou precisar mostrar uma definição alternativa do produto interno. Vamos pensar na norma ao quadrado da diferença de dois vetores:

$$
\|(a-b)\|^2 = (a - b) \cdot (a-b) = a \cdot a + b \cdot b - 2 a \cdot b = \|a\|^2 + \|b\|^2 - 2(a \cdot b)
$$

Agora, vamos lembrar da lei dos cossenos, que, usando a notação dessa figura:
...
Então, o angulo entre dois vetores e a correlação entre eles são a mesma coisa se a média deles for zero. Dai o nome correlação de vetores para o cosseno do angulo entre eles.
...
Agora divita-se, calcule os $\Delta z$ ao longo da filogenia, e use a correlação de vetores para investigar seus dados. Os $\Delta z$ são todos na mesma direção? Como os $\Delta z$ nos teminais se alinham com as respectivas linhas de menor resistência? Pense em quais perguntas você é capaz de responder com esses dados agora.